...
func validateShortcutPatch(shortcut *shortcut) error {
	if shortcut.Patch != "" && shortcut.Patch != patchDelete && shortcut.Patch != patchMerge && shortcut.Patch != patchReplace {
		return errors.Errorf("Patch of \"%s\" Shortcut must be either \"%s\", \"%s\" or \"%s\" (got \"%s\")",
			shortcut.Name, patchDelete, patchMerge, patchReplace, shortcut.Patch)
	}
	return nil
}
